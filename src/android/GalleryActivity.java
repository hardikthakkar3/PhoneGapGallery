...
public class GalleryActivity extends Activity implements OnItemClickListener {
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
		...
		Uri uri = Uri.parse(MediaStore.Images.Media.EXTERNAL_CONTENT_URI + "/"
				+ imageID);
        String fileURL = uriToStringConvert(uri);

		getIntent().setData(uri);
        getIntent().putExtra("fileURL",fileURL);
		setResult(RESULT_OK, getIntent());

		finish();
	}

    public String uriToStringConvert(Uri uri) {

        try {
            String[] projection = {MediaStore.Images.Media.DATA};
            Cursor cursor = GalleryActivity.this.getContentResolver().query(uri, projection, null, null, null);
